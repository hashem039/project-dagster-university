...
def trips_by_week(context: dg.AssetExecutionContext, database: DuckDBResource) -> None:
    ...
    """
    week_to_fetch = context.partition_key

    query = f"""
    SELECT 
        (pickup_datetime::DATE - EXTRACT(dow FROM pickup_datetime)::INT)::VARCHAR AS period, 
        COUNT(*) AS num_trips, 
        SUM(passenger_count) AS passenger_count, 
        SUM(total_amount) AS total_amount, 
        SUM(trip_distance) AS trip_distance 
    FROM trips 
    WHERE pickup_datetime >= '{week_to_fetch}'::DATE 
        AND pickup_datetime < '{week_to_fetch}'::DATE + INTERVAL '7 days'
    GROUP BY period 
    ...
    with database.get_connection() as conn:
        ...
        result_df.to_csv(constants.TRIPS_BY_WEEK_FILE_PATH)
